...
public class DeleteProductControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (a != null) {
            if (a.getBrandID() != 0 || a.getIsAdmin() == 1) {
                mp.deleteProduct(id);
                String brandID = request.getParameter("brandID");
                request.setAttribute("mess", "Your new product has been deleted successfully");
                request.getRequestDispatcher("loadmanageproduct?bid=" + brandID).forward(request, response);
            }
        } else {
            PrintWriter out = response.getWriter();
            out.print("You must be login with Brand account to do this action");
        }
    }
